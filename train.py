...
def train(enviornment, agent, episodes, valInterval, val):
    ...
    for episode in range(1, episodes + 1):
        ...
        print(f"\rEpisode: {episode}\t Score: {score}\t50-Episode Average:{(sum(metrics['Train Score'][-50:]) / 50)}", end='')
...
def main():
    ...
    parser.add_argument("--val", type=bool, default=False, help="Whether to validate or not after training")
    ...
    agent = AgentModule(args.batch_size, args.learning_rate, 0.999, 2000, device)
    ...
    trainMetrics = train(env, agent, args.episodes, args.val_interval, args.val)
